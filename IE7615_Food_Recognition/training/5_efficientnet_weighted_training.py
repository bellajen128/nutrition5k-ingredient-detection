...
# OUTPUT_DIR = BASE_DIR / "efficientnet_weighted_outputs"

OUTPUT_DIR = BASE_DIR / "efficientnet_b3_weighted_outputs"
...
# =============================================================================
# Step 4.5: Calculate Class Weights (使用 Median Frequency Balancing)
# =============================================================================
print("\n" + "="*80)
print("Calculating Class Weights (MFB for Stability)")
print("="*80)

num_dishes = len(train_df)
label_counts = np.zeros(num_classes)

# 統計每個類別的總出現次數
for label_json in tqdm(train_df['label_json'], desc="Counting Labels"):
    labels = np.array(json.loads(label_json))
    label_counts += labels

# 1. 計算類別頻率 (P_i)
class_freq = label_counts / num_dishes

# 2. 計算中位數頻率 (Median Frequency)
# 排除頻率為 0 的類別
non_zero_freq = class_freq[class_freq > 0]
if len(non_zero_freq) == 0:
    # 如果所有類別頻率都為 0，這是一個嚴重問題，但我們必須處理
    median_freq = 1.0
else:
    median_freq = np.median(non_zero_freq)

# 3. 計算 MFB 權重: W_i = Median_Freq / P_i
# 對於未出現的類別 (P_i=0)，設定一個較高的但非無限的權重，例如最大權重。
weights = np.where(class_freq > 0, median_freq / class_freq, median_freq * 100)
# 限制最大權重，防止單一類別崩潰整個訓練
weights = np.clip(weights, 1.0, 50.0) # 最大權重設為 50

# 將權重轉換為 PyTorch 張量並移到 GPU
class_weights = torch.tensor(weights, dtype=torch.float32).to(device)

print(f"✓ MFB Class weights calculated. (Median Freq: {median_freq:.4f})")
...
# 尋找米飯的索引並顯示其權重，以便驗證
try:
    rice_idx = vocab.index('white rice') 
    print(f"  'white rice' index: {rice_idx}, weight: {class_weights[rice_idx].item():.2f}")
except ValueError:
    print("  'white rice' label not explicitly found in vocab for display.")
...
class EfficientNetMultiLabel(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=True, dropout=0.3):
        ...
        # Get backbone output dimension (EfficientNet-B3: 1536)
...
LABEL_THRESHOLD = 0.2    # For binary prediction
...
# Loss function for multi-label classification (Applying class weights)
# pos_weight 參數將使用我們計算出的類別權重，解決類別不平衡問題。
criterion = nn.BCEWithLogitsLoss(pos_weight=class_weights)
print(f"✓ Loss function: BCEWithLogitsLoss (with Class Weights applied)")
...
if best_model_path.exists():
    ...
    # === [新增: 保存預測結果和標籤 (供閾值優化使用)] ===
    TEST_OUTPUT_DIR = OUTPUT_DIR / 'threshold_data'
    TEST_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    np.save(TEST_OUTPUT_DIR / 'test_preds.npy', test_preds)
    np.save(TEST_OUTPUT_DIR / 'test_labels.npy', test_labels)
    
    print(f"✓ Test predictions and labels saved for sweep analysis in {TEST_OUTPUT_DIR}")
    # =======================================================
else:
